...
class _LetterScreenState extends State<LetterScreen> with SingleTickerProviderStateMixin {
  ...
  late final _float = AnimationController(vsync: this, duration: const Duration(milliseconds: 1800))..repeat(reverse: true);
  String _line = '';
  int _bounce = 0;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (widget.ask) {
        ...
        _say('{name}! You got a message from ${l.from}! Would you like to hear what ${l.from} has to say?');
      } else {
        _listen();
      }
    });
  ...
  Future<void> _listen() async {
    ...
    if (audio != null && File(audio).existsSync()) {
      ...
      await Voice.say('${l.from} says: ${l.text}');
    }
    if (!mounted) return;
    setState(() {
    ...
    _say(l.hugSent ? 'Wasn\'t that lovely?' : 'Aww! Would you like to send ${l.from} a big hug back?');
  }
  ...
  Future<void> _hug() async {
    ...
    _say('Whoosh! Your hug is flying to ${l.from}! ${l.from} will feel it right in their heart.');
  }
  ...
  Widget build(BuildContext context) {
    final mood = switch (_stage) {
      ...
      _Stage.listening => Mood.happy,
      _Stage.after => Mood.happy,
      _Stage.hugged => Mood.cheer,
    };
    return Scaffold(
      body: Meadow(
        child: SafeArea(
            ...
            child: Column(
              children: [
                ...
                Stack(
                  ...
                  children: [
                    Bibi(mood: mood, size: 250, bounce: _bounce, onTap: Sfx.giggle),
                    if (_stage == _Stage.listening)
                      Positioned(
                        ...
                        child: AnimatedBuilder(
                          animation: _float,
                          builder: (_, c) => Transform.translate(offset: Offset(0, -_float.value * 12), child: c),
      ...
      body: Meadow(
        child: SafeArea(
          child: ListenableBuilder(
            listenable: app,
            builder: (context, _) {
              final list = app.letters.where((l) => !l.bedtime || l.opened).toList().reversed.toList();
              return Padding(
                padding: const EdgeInsets.all(16),
                child: Column(
                  children: [
                    ...
                    if (list.isEmpty)
                      Expanded(
                        child: Center(
                          child: Bubble('No letters yet. When someone who loves you sends one, I will bring it straight to you!'),
                        ),
                      )
                    else
                      Expanded(
                        child: GridView.count(
                          ...
                          children: [
                            for (final l in list)
                              Chunky(
                                color: l.opened ? C.paper : const Color(0xFFFFE9EE),
                                shadow: l.opened ? C.shadow : const Color(0xFFF3B3C3),
                                radius: 26,
                                padding: const EdgeInsets.all(10),
                                onTap: () => Navigator.of(context).push(softRoute(LetterScreen(letter: l, ask: false))),
                                child: Stack(
                                  clipBehavior: Clip.none,
                                  children: [
                                    Column(mainAxisSize: MainAxisSize.min, children: [
                                      Text(l.opened ? '💌' : '✉️', style: const TextStyle(fontSize: 58)),
                                      Text(l.from, style: T.d(20)),
                                      if (l.hugSent) const Text('🤗', style: TextStyle(fontSize: 20)),
                                    ]),
                                    if (!l.opened) const Positioned(right: -4, top: -4, child: GlowDot()),
                                  ],
                                ),
                              ),
                          ],
  ...
  Widget build(BuildContext context) => AnimatedBuilder(
        ...
        builder: (_, _) => Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: C.berry.withValues(alpha: .6), blurRadius: 6 + _c.value * 12, spreadRadius: _c.value * 4)],
          ),
        ),
      );
}
