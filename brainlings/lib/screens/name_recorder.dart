...
class VoiceRecorder extends StatefulWidget {
  const VoiceRecorder({super.key, required this.fileName, required this.onSaved, this.existing, this.maxSeconds = 120});
  ...
  Future<void> _start() async {
    ...
    try {
      if (!await _rec.hasPermission()) {
        setState(() => _error = 'We need the microphone to record. You can allow it in your phone settings.');
        return;
      }
      final dir = await getApplicationDocumentsDirectory();
      final path = '${dir.path}/${widget.fileName}-${DateTime.now().millisecondsSinceEpoch}.m4a';
      await Voice.stop();
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
          ...
          children: [
            Expanded(
              child: FilledButton.icon(
                ...
                label: Text(_recording ? 'Stop' : (_path == null ? 'Record' : 'Record again')),
              ),
            ),
            if (_path != null && !_recording) ...[
              const SizedBox(width: 10),
              FilledButton.icon(
                style: FilledButton.styleFrom(backgroundColor: C.leaf, minimumSize: const Size(0, 54), textStyle: T.d(18)),
        ...
        if (_error != null) Padding(padding: const EdgeInsets.only(top: 8), child: Text(_error!, style: T.b(14, color: C.berryDeep))),
      ],
    );
  }
}
