...
const circleSuggestions = ['Mummy', 'Daddy', 'Grandma', 'Grandad', 'Nana', 'Auntie', 'Uncle', 'Big Sister', 'Big Brother'];
...
class _SetupScreenState extends State<SetupScreen> {
  ...
  void _go(int p) {
    ...
    setState(() => _page = p);
    _pages.animateToPage(p, duration: const Duration(milliseconds: 450), curve: Curves.easeOutCubic);
  }
  ...
  Widget build(BuildContext context) {
    final kid = _name.text.trim().isEmpty ? 'your little one' : _name.text.trim();
    return Scaffold(
      body: Meadow(
        child: SafeArea(
          child: Column(
            children: [
              ...
              Expanded(
                child: PageView(
                  ...
                  children: [
                    _card([
                      const Center(child: Bibi(size: 170, showGrowth: false)),
                      const Eyebrow('Welcome to Brainlings'),
                      Text('Learn, grow, and hear from the people who love you.', style: T.d(30)),
                      ...
                      Text('This takes about a minute. No ads, ever.', style: T.b(16, color: C.inkSoft)),
                    ...
                    _card([
                      ...
                      Text('A first name or a nickname is perfect. That is all we need.', style: T.b(15, color: C.inkSoft)),
                      const SizedBox(height: 14),
                      TextField(
                        controller: _name,
                        textCapitalization: TextCapitalization.words,
                        style: T.d(22),
                        decoration: const InputDecoration(hintText: 'e.g. Chris'),
                      ...
                      Wrap(spacing: 8, runSpacing: 8, children: [
                        for (final p in {...circleSuggestions, ...app.circle})
                          FilterChip(
                            ...
                            selectedColor: C.sun,
                            onSelected: (on) => setState(() => on ? app.circle.add(p) : app.circle.remove(p)),
                          ),
                      ]),
                      const SizedBox(height: 10),
                      Row(children: [
                        Expanded(
                          child: TextField(
                            controller: _custom,
                            textCapitalization: TextCapitalization.words,
                            decoration: const InputDecoration(hintText: 'Someone else (e.g. Auntie Mia)'),
                          ),
                        ),
                        const SizedBox(width: 8),
                        IconButton.filled(
                          style: IconButton.styleFrom(backgroundColor: C.leaf),
                          onPressed: () {
                            final v = _custom.text.trim();
                            if (v.isNotEmpty && !app.circle.contains(v)) setState(() => app.circle.add(v));
                            _custom.clear();
                          },
                    ...
                    _card([
                      ...
                      const SizedBox(height: 8),
                      Text('Skip it if you like. We will say the name ourselves.', style: T.b(13, color: C.inkSoft)),
                      const SizedBox(height: 22),
                      _next(app.nameClipPath == null ? 'Skip for now' : 'Next', () => _go(4)),
                    ]),
                    _card([
                      const Eyebrow('Healthy play'),
                      Text('When should the creature get sleepy?', style: T.d(28)),
                      const SizedBox(height: 6),
                      Text(
                      ...
                      const SizedBox(height: 14),
                      Text('${app.bedtimeMinutes} minutes a day', style: T.d(26, color: C.lilacDeep)),
                      Slider(
                        ...
                        onChanged: (v) => setState(() => app.bedtimeMinutes = v.round()),
                      ),
                      const SizedBox(height: 10),
                      Container(
  ...
  Widget _next(String label, VoidCallback? onTap, {bool big = false}) => Chunky(
        ...
        onTap: onTap,
        child: Text(label, style: T.d(big ? 22 : 20, color: big ? Colors.white : C.ink)),
      );
}
