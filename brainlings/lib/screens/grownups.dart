...
class _GrownUpsScreenState extends State<GrownUpsScreen> {
        ...
        child: SafeArea(
          child: ListenableBuilder(
            ...
            builder: (context, _) => ListView(
              ...
              children: [
                Row(children: [
                  RoundIcon(icon: Icons.arrow_back_rounded, label: 'Back to play', onTap: () => Navigator.of(context).pop()),
  ...
  Widget _progress() {
    ...
    final growth = switch (app.look) {
      GrowthLook.baby => '${app.creatureName} is still a baby. A little more learning and it will start to grow.',
      GrowthLook.starry => '${app.creatureName} is growing sparkly stars, because $kid loves numbers.',
      GrowthLook.bookish => '${app.creatureName} is wearing a book hat, because $kid loves letters.',
      GrowthLook.balanced => '${app.creatureName} has stars and a book hat: $kid is learning a bit of everything.',
    };
    ...
    return GrownCard(children: [
      ...
      GridView.count(
        ...
        children: [
          ...
          _stat('${p[Skill.teaching]! ~/ 2}', 'times they taught ${app.creatureName}'),
      ...
      Container(
        ...
        decoration: BoxDecoration(color: const Color(0xFFFFF6D6), borderRadius: BorderRadius.circular(16)),
      ...
      if (recent.isNotEmpty) ...[
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          for (final f in recent)
            Chip(
              label: Text('${_feelEmoji(f.split('|').last)} ${_day(f.split('|').first)}', style: T.b(14)),
  ...
  String _feelEmoji(String f) => const {'happy': '😊', 'sad': '😢', 'tired': '😴', 'cross': '😠'}[f] ?? '🙂';

  String _day(String key) {
    if (key == AppState.dayKey()) return 'Today';
    if (key == AppState.dayKey(DateTime.now().subtract(const Duration(days: 1)))) return 'Yesterday';
  ...
  Widget _letters() {
    ...
    return GrownCard(children: [
      ...
      const SizedBox(height: 12),
      FilledButton.icon(
        style: FilledButton.styleFrom(backgroundColor: C.berry, minimumSize: const Size.fromHeight(54), textStyle: T.d(18)),
        onPressed: () => Navigator.of(context).push(softRoute(const RecordLetterScreen())),
      ...
      for (final l in list)
        Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFF6F4FE), borderRadius: BorderRadius.circular(16)),
          child: Row(children: [
            Text(l.bedtime ? '🌙' : (l.audioPath != null ? '🎙️' : '✉️'), style: const TextStyle(fontSize: 28)),
            const SizedBox(width: 10),
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('From ${l.from}${l.bedtime ? ' (bedtime)' : ''}', style: T.d(17)),
  ...
  String _status(Letter l) {
    if (!l.opened) return l.bedtime ? 'Waiting for bedtime' : 'Waiting in $kid\'s letterbox';
    final times = l.plays == 1 ? 'once' : (l.plays == 2 ? 'twice' : '${l.plays} times');
  ...
  Widget _settings() {
    return GrownCard(children: [
      ...
      const SizedBox(height: 16),
      Text('Sleepy time after ${app.bedtimeMinutes} minutes a day', style: T.d(18)),
      Slider(
      ...
      TextButton(
        ...
        child: Text('Start again from the beginning', style: T.b(15, color: C.berryDeep, w: FontWeight.w800)),
      ),
    ]);
  }
  ...
  Future<void> _confirmReset() async {
    final yes = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: Text('This clears $kid\'s creature, stars, stickers and letters on this phone. It cannot be undone.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(c, false), child: const Text('Keep everything')),
          TextButton(onPressed: () => Navigator.pop(c, true), child: const Text('Start again')),
        ],
      ),
    );
    if (yes == true && mounted) {
      app.resetAll();
      Navigator.of(context).pushAndRemoveUntil(softRoute(const SetupScreen()), (_) => false);
    }
  }
}
...
class _RecordLetterScreenState extends State<RecordLetterScreen> {
  ...
  void _send() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(_bedtime ? 'Saved for bedtime. ${app.creatureName} will play it as it falls asleep.' : '${app.creatureName} will bring it to ${app.displayName} next time they play.'),
    ));
  }
          ...
          child: ListView(
            ...
            children: [
              Align(
                ...
                child: RoundIcon(icon: Icons.arrow_back_rounded, label: 'Back', onTap: () => Navigator.of(context).pop()),
              ),
              const SizedBox(height: 16),
              GrownCard(children: [
                ...
                Text('A few warm words is plenty. Up to two minutes.', style: T.b(14, color: C.inkSoft)),
                const SizedBox(height: 10),
                VoiceRecorder(fileName: 'letter', onSaved: (p) => setState(() => _audio = p)),
                const SizedBox(height: 18),
                Text('Or write it, and ${app.creatureName} will read it aloud', style: T.d(18)),
                const SizedBox(height: 8),
                TextField(
                  controller: _text,
                  minLines: 2,
                  ...
                  decoration: InputDecoration(hintText: 'I\'m so proud of you, $kid. Love you to the moon!'),
                ),
                const SizedBox(height: 12),
                SwitchListTile(
                  ...
                  subtitle: Text('Played when ${app.creatureName} falls asleep', style: T.b(13, color: C.inkSoft)),
                ),
                const SizedBox(height: 12),
                FilledButton.icon(
                  style: FilledButton.styleFrom(backgroundColor: C.leaf, minimumSize: const Size.fromHeight(56), textStyle: T.d(19)),
